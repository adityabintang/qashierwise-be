...
const DURATIONS = [
  { key: "1_month", label: "1 Bulan", suffix: "/bln" },
  { key: "3_months", label: "3 Bulan", suffix: "/3bln" },
  { key: "1_year", label: "1 Tahun", suffix: "/thn" },
] as const;
...
export function Pricing() {
  ...
  async function startProCheckout(forcedDuration?: string) {
    ...
    try {
      ...
      setError(data?.error?.message || "Gagal membuat sesi checkout. Silakan coba lagi.");
    } catch {
      ...
      <Container>
        ...
        <div className="grid sm:grid-cols-2 gap-5 max-w-[880px] mx-auto">
          ...
          <div className="reveal reveal-delay-2 bg-gradient-to-b from-[#1B1430] to-[#2D1B5C] text-white rounded-[28px] p-8 transition-all hover:-translate-y-1 shadow-purple relative">
            ...
            <div className="text-[13.5px] text-white/60 mt-1.5">Hingga 2 outlet · delivery + QRIS</div>
            <div className="my-[22px]">
              ...
              {billing !== 0 && (
                <div className="mt-1.5 text-[13px] text-white/60 font-medium">{rupiah(proPerMonth)}/bln</div>
              )}
            </div>
            <ul className="list-none p-0 m-0 grid gap-2.5">
              {proFeatures.map((t) => (
            ...
            >
              {loading ? "Memproses…" : <>Pilih Pro <ArrowIcon /></>}
            </button>
          </div>
        </div>
      </Container>
